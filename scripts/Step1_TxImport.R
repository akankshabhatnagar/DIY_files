...
library(EnsDb.Hsapiens.v86) #Use library(EnsDb.Mmusculus.v79) for mice data
library(biomaRt) #Alternative for annotations
library(beepr) #just for fun
library(datapasta) #Paste data into R from clipboard


# read in your study design ----
setwd("/Users/abhatnagar/Library/CloudStorage/Dropbox-HELLERLAB/Akanksha Bhatnagar/Mac/Desktop/DIYfiles")
#there are LOTS of ways to read data into R, but the readr package (from tidyverse) is one of the simplest
#read_tsv reads your file into a tibble (tidy table)
...
#path basically joins arguments with a / to make a pth
...
myMart <- useMart(biomart="ENSEMBL_MART_ENSEMBL",dataset = "mmusculus_gene_ensembl")

#now grab the ensembl annotations for dog
mus.anno <- useMart(biomart="ENSEMBL_MART_ENSEMBL", dataset = "mmusculus_gene_ensembl")
mus.attributes <- listAttributes(mus.anno)

Tx.mus <- getBM(attributes=c('ensembl_transcript_id_version',
                         'external_gene_name'),
            mart = mus.anno)

Tx.mus <- as_tibble(Tx.mus)
#we need to rename the two columns we just retreived from biomart
Tx.mus <- dplyr::rename(Tx.mus, target_id = ensembl_transcript_id_version, 
                    gene_name = external_gene_name)
View(Tx.mus)

# OPTIONAL: Use BiomaRt to retrieve upstream sequences----
#Use ?getSequence function in biomaRt to retrieve sequences
#Type must be an attribute in myMart. Upstream start is set to 1kb upstream of promoter.
#Can replae mart as myMart variable
Upstreamseq <- getSequence(id=c("Cdk1", "Cdk2", "Cdk5"),
...
#txOut TRUE = transcripts as output
#txOut FALSE = Gene names as output (good for human and mouse data)
...
class(Txi_gene)
names(Txi_gene)
#column counts = total reads, abundance = TPM
